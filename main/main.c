...
void init_wifi(void *args){
	esp_err_t err;
	ScreenCommand err_message_to_screen;
	...
	ScreenCommand wifi_name_command = {
		.command = 'p'
	};
	strcpy(wifi_name_command.message, (char *) ap_record.ssid);
	xQueueSend(screenManagerQueue, &wifi_name_command, 0);
	vTaskDelete(NULL);
}
		...
		if (xQueueReceive(screenManagerQueue, &recived_message, 0) == pdTRUE){
			switch (recived_message.command) { // screen manager command palette
				case 'p': // print terminal style
					...
					ssd1306_display_text(&device, page_counter, recived_message.message, strlen(recived_message.message), false);
					page_counter++;
					break;
